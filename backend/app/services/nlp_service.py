...
def get_nlp():
    ...
    `spacy` is imported inside the function rather than at module
    scope: importing it costs seconds, and this module is reachable
    from the router imports, so at module scope every `import main`
    paid that cost before the app could start. The model load below
    already dwarfs the import, and lru_cache means both happen once.
    """
    import spacy
